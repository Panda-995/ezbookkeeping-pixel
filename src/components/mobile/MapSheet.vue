<template>
    <f7-sheet swipe-to-close swipe-handler=".swipe-handler" class="map-sheet" style="height:auto"
        ...
        <f7-toolbar class="toolbar-with-swipe-handler">
            ...
            <div class="left">
                <f7-link icon-f7="minus" :aria-label="tt('Zoom Out')" :class="{ 'disabled': !map?.allowZoomOut() }" @click="map?.zoomOut()"></f7-link>
                <f7-link icon-f7="plus" :aria-label="tt('Zoom In')" :class="{ 'disabled': !map?.allowZoomIn() }" @click="map?.zoomIn()"></f7-link>
            </div>
