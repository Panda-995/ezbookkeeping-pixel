    ...
    <f7-page ptr @ptr:refresh="reloadCacheStatistics">
        <f7-navbar>
            ...
            <f7-nav-right :class="{ 'disabled': loading }">
                <f7-link icon-f7="ellipsis" :aria-label="tt('More')" :class="{ 'disabled': loading || ((!isSupportedFileCache || !fileCacheStatistics) && !exchangeRatesCacheSize) }" @click="showMoreActionSheet = true"></f7-link>
            </f7-nav-right>
        </f7-navbar>
